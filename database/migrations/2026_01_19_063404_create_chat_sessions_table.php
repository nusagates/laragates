...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('chat_sessions', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('customer_id')->index('chat_sessions_customer_id_foreign');
            $table->unsignedBigInteger('assigned_to')->nullable()->index('chat_sessions_assigned_to_foreign');
            $table->boolean('pinned')->default(false);
            $table->enum('priority', ['normal', 'high', 'vip'])->default('normal');
            $table->enum('status', ['bot', 'pending', 'open', 'closed'])->nullable()->default('open');
