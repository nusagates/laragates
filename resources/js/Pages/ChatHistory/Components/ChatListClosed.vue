...
const filtered = computed(() => {
  const closedOnly = chatStore.sessions.filter(session => session.status === 'closed')
  
  if (!query.value) return closedOnly

  return closedOnly.filter(session =>
    ...
    <div class="chat-list">
      ...
      <div class="chat-scroll">
        ...
        <div v-else-if="filtered.length === 0" class="empty-state">
          <v-icon size="48" color="grey">mdi-archive-outline</v-icon>
          <p class="empty-text">No closed chats found</p>
        </div>
      </div>
    </div>
  </aside>
